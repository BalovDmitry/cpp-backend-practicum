...
StringResponse RequestHandler::HandleRequest(StringRequest&& req) {
    ...
    if (req.method() == http::verb::get) {
        ...
        if (IsApiRequest(req)) {
            SetResponseDataApi(splittedRequest, GetRequestTypeApi(splittedRequest), bodyText, status);
    ...
    auto response =  text_response(status, bodyText, contentType);
...
void RequestHandler::SetResponseDataApi(
    const std::vector<std::string>& splittedRequest, 
    RequestTypeApi requestType, 
    std::string& bodyText, 
    http::status& status) {
